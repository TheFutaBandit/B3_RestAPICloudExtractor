...
def process_csv(input_csv, output_csv):
            ...
            if commit_shas:
                for commit_sha in commit_shas:
                    ...
                    for file in files_changed:
                        results.append({'Bug ID': bug_id, 'File': file})
            else:
    ...
    with open(output_csv, mode='w', newline='', encoding='utf-8') as file:
        writer = csv.DictWriter(file, fieldnames=['Bug ID', 'File'])
        writer.writeheader()
        writer.writerows(results)

def main():
    input_csv = 'result.csv'  # Input CSV file containing the 'Bug ID' column
    output_csv = 'bugIdMapped.csv'  # Output CSV file to store the results
